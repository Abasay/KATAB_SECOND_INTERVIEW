...
const Signin = () => {
  ...
  const [passPhrase, setPassPhrase] = useState<boolean>(false);
  const [stringPassPhrase, setOpenStringPassPhrase] = useState<boolean>(false);
  ...
  const verifyOTP = async () => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASEURL}/auth/user/verifyotp`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
                        ...
                        {!forgetPasswd && (
                          <>
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setData({ ...data, passPhrase: e.target.value })
                              }
                              className="w-full border-b border-stroke !bg-white pb-3.5 focus:border-waterloo  focus-visible:outline-none dark:border-strokedark dark:!bg-black dark:focus:border-manatee dark:focus:placeholder:text-white lg:w-1/2"
                            />
                          </>
